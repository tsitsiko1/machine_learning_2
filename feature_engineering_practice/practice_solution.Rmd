...
hr_df$number_project_cat <-recode(hr_df$number_project,`2`="Low",`3`="Low",`4`="Medium",`5`="Medium",`6`="High", `7`= "Very_High")
hr_df$time_spend_company_cat <- recode(hr_df$time_spend_company,`2`="Low",`3`="Medium",`4`="Medium",`5`="Medium",`6`="High", `7`= "High", `8`= "Very_High", `10`= "Very_High")
```

**Note: Some of you got an error in the stepwise methods related to the name of the features. This is due to I accidently included some whitespaces in the `number_project_cat` and `time_spend_company_cat` features; e.g., Very High. I have corrected this issue in the previous code chunk, so it should work now.**

## Hunting NAs

There is not any NULL values, so we can skip this step. You can see by summarizing the dataset or you can use the `is.na` function to identify the null values.
...
```{r Outlier Detection}
ggplot(hr_df, aes(x="",y=time_spend_company))+ geom_boxplot(width=0.1) + theme(axis.line.x=element_blank(),axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.text.x=element_blank(),legend.position="none")
```

We don't want these extreme cases to affect or bias the training process, so the best is to remove them.
We can apply some metric (i.e., the Z-score) to detect and remove these points. The `boxplot` function itself provides a way to remove them:
...
for (col in names(hr_df)) {
  if (is.numeric(hr_df[[col]]) && col != "left"){
    print(ggplot(hr_df, aes_string(y=col))+ geom_boxplot(width=0.1) + theme(axis.line.x=element_blank(),axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.text.x=element_blank(),legend.position="none"))
    to_remove <- boxplot.stats(hr_df[[col]])$out
    hr_df <- hr_df[!hr_df[[col]] %in% to_remove, ]
  }
}
```
...
We first fit a `glm` model to the initial dataset to have a baseline to evaluate the impact of the data cleaning and feature engineering.
...
initial_training$left <- factor(initial_training$left, levels = c("0", "1"), labels = c("No", "Yes"))
initial_test$left <- factor(initial_test$left, levels = c("0", "1"), labels = c("No", "Yes"))

# Train
initial.lm.mod <- train(left ~ ., # Formula: Target variable ~ predictors
...
cm_initial <- confusionMatrix(initial.lm.mod.pred, initial_test$left, positive = "Yes")
print(cm_initial)
...
weights<- data.frame(chi.squared(left~., training)) # Chi-squared computation
...
chi_squared.lm.mod <- train(left ~ ., data = training[append(chi_squared_features, "left")], 
               method = "glm", 
               metric = "Accuracy")


chi_squared.lm.mod.pred <- predict(chi_squared.lm.mod, test[,-which(names(test) == "left")])

cm <- confusionMatrix(chi_squared.lm.mod.pred, test$left, positive = "Yes")
print(cm)
```

As shown by the confusion matrix, we have improved a little bit the performance of the model while reducing the number of features.
...
ig.lm.mod <- train(left ~ ., data = training[append(information_gain_features, "left")],
               method = "glm", 
               metric = "Accuracy")


ig.lm.mod.pred <- predict(ig.lm.mod, test[,-which(names(test) == "left")])

cm <- confusionMatrix(ig.lm.mod.pred, test$left, positive = "Yes")
print(cm)
```

Information gain seems to improve a little bit more the performance of the model. Based on these results, we filter the training and validation set with the Information Gain features.

```{r Information Gain Selection}
ig_training <- training[append(information_gain_features, "left")]
ig_test <- test[append(information_gain_features, "left")]
```
...
backward.lm.mod <- train(left ~ ., data = training, 
               method = "glmStepAIC", 
               direction = "backward",
               trace = FALSE,
               metric = "Accuracy",
...
backward.lm.mod.pred <- predict(backward.lm.mod, test[,-which(names(test) == "left")])

cm <- confusionMatrix(backward.lm.mod.pred, test$left, positive = "Yes")
print(cm)
```
...
forward.lm.mod <- train(left ~ ., data = training, 
                                       method = "glmStepAIC", 
                                       direction = "forward",
                                       trace=FALSE,
                                       metric = "Accuracy",
...
forward.lm.mod.pred <- predict(forward.lm.mod, test[,-which(names(test) == "left")])

cm <- confusionMatrix(forward.lm.mod.pred, test$left, positive = "Yes")
print(cm)
```

This process is not able to further refine the feature set (more than the refinement already performed by the feature selection process). So, we will take the IG-cleaned training for the following step.
...
For this exercise, we are going to make use of the <a href="https://cran.r-project.org/web/packages/glmnet/index.html">`glmnet`</a> library. Take a look to the library and fit a glmnet model making use of `caret` training function for Ridge Regression, using the grid of lambda values provided.

```{r Ridge Regression}
lambdas <- 10^seq(-2, 1, by = .1)

ridge.mod <- train(left ~ ., data = ig_training, 
               method = "glmnet", 
               metric = "Accuracy",
...
ridge.pred= predict(ridge.mod, s=bestlam, ig_test[,-which(names(ig_test) == "left")])
cm <- confusionMatrix(ridge.pred, ig_test$left, positive = "Yes")
print(cm)
```
...
lasso.mod <- train(left ~ ., data = ig_training, 
               method = "glmnet", 
               metric = "Accuracy",
...
paste("Best Lambda value from CV=", bestlam)
lasso.pred= predict(lasso.mod, s=bestlam, ig_test[,-which(names(ig_test) == "left")])
cm <- confusionMatrix(lasso.pred, ig_test$left, positive = "Yes")
print(cm)
```

Lasso is able to improve the results achieved by the Information Gain selection by focusing only on a small subset of representative features (nullifying the rest of the coefficients). 
