...
packages = c("kableExtra","devtools","e1071", "caret", "ROCR", "pROC")
...
```{r}
appPrior = log(nrow(appFreqs) / (nrow(appFile) + nrow(otherFile))) # Number of Tweets in app File / Total number of Tweets (sum of the number of tweets in both files)
otherPrior = log(nrow(otherFile) / (nrow(appFile) + nrow(otherFile))) # Number of Tweets in other File / Total number of Tweets (sum of the number of tweets in both files)
```
...
Now, let's loop through the file to compute the MAP (maximum A Posteriori Probability) and thus, determine which class the tweet belongs to:
...
## Data Preparation
The original dataset (in CSV) data can be downloaded from this [link](http://boston.lti.cs.cmu.edu/classes/95-865-K/HW/HW3/movie-pang02.zip).
...
Using the functions in `tm` package, create a corpus representation of the dataset.
```{r}
...
Separate data into training and test sets (75% for training and 25% for test).
```{r}
# Your code here
...
Tip: confusion matrix need the predictions as classes. Take a look to the "type" parameter in the `predict` function.
```{r}
# Your code here
```
...
Compute the Area Under the ROC (take a look to the `pROC` library)
```{r message=FALSE, warning=FALSE}
# Your code here
